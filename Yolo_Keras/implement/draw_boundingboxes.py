...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            print(box)
            top = round(top * Ry).astype('int32')
            left = round(left * Rx).astype('int32')
            bottom = round(bottom * Ry).astype('int32')
            right = round(right * Rx).astype('int32')
            ...
            if c == 4:
                try:
                    ...
                    text = text[:2]+'000'
                    cv2.putText(image, text[:5], (left, top), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
                    print(text)
                except:
                    print('sth wrong')
            # x, y, w, h = box
...
yolo = YOLO()
img = cv2.imread('test_image/red_bull/VID_20191124_091922 01.jpg')
# shape = img.shape
# img = cv2.resize(img,(416,416))
r_image, ObjectsList = yolo.detect_img(img)
# r_image = cv2.resize(r_image,(shape[1],shape[0]))
cv2.rectangle(r_image,(734,378),(1081,893),(0,255,255),4)
r_image = cv2.cvtColor(r_image,cv2.COLOR_RGB2BGR)
cv2.imshow('coca',r_image)
cv2.waitKey()
cv2.imwrite('coca_iou.jpg',r_image)
